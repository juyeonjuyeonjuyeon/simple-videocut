...
const source = join(testEnv.root, 'close-input.mp4')
execFileSync(ffmpeg, [
  '-y', '-f', 'lavfi', '-i', 'color=c=black:s=32x32:r=1', '-t', '0.1',
...
try {
  ...
  await page.evaluate(async (inputBytes) => {
    const bridge = globalThis.simplecutDesktop
    if (!bridge) throw new Error('네이티브 FFmpeg 연결을 찾지 못했습니다.')
    await bridge.writeFile('close-input.mp4', new Uint8Array(inputBytes))
