...
- **로컬 AI 배경 제거** — 사진을 업로드하지 않고 기기 안에서 피사체를 분리하며 민감도 조절·오프라인 캐시·색상 방식 대체 처리
- **영역 모자이크** — 사진·영상에서 여러 영역을 지정하고 픽셀 크기를 조절하며 미리보기·프로젝트 복원·MP4 내보내기에 동일 적용
...
React 18 · TypeScript · Vite · Zustand(상태 관리) · ONNX Runtime Web(로컬 AI) · ffmpeg.wasm(웹) · Electron/FFmpeg(VideoToolbox, macOS)
...
- **AI 배경 제거**는 Apache-2.0 라이선스의
  [ormbg ONNX 모델](https://huggingface.co/onnx-community/ormbg-ONNX)을 브라우저 전용
  ONNX Runtime에서 실행합니다. 모델은 처음 사용할 때 한 번 내려받아 브라우저 캐시에
  보관하며 사진 픽셀은 외부 서버로 전송하지 않습니다. 다운로드나 실행이 불가능하면
  가장자리 연결 색상 방식으로 자동 전환합니다.
