...
export default function App() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const modalOpen = showExport || showHelp || showCropEditor || showMosaicEditor || showShapes || showStickers || showCaptions || showShowcasePreview || showProjectHome || projectDialogMode !== null
  ...
  return (
    <div className={`app${compactPanels ? ' app--compact-panels' : ''}`} style={appStyle} onDragEnter={onDragEnter} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
      ...
      <main className={`stage${inspectorOpen ? '' : ' stage--inspector-hidden'}${mediaPanelOpen ? '' : ' stage--media-hidden'}`}>
        ...
        <section className="stage__preview">
          ...
          <div className={`transport${selection ? ' transport--selection' : ''}`}>
            ...
            <button className="btn btn--sm transport__insert-action" onClick={() => { setPlaying(false); setShowCaptions(true) }}><Icon name="captions" />{t('자막', 'Captions')}</button>
      ...
      {showCaptions && <CaptionDialog projectName={activeProjectName} onClose={() => setShowCaptions(false)} onImported={() => {
